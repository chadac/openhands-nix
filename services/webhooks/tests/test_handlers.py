...
from openhands_webhooks.app import app


@pytest.fixture
def client():
    """Create a test client with mocked DB and messaging."""
    with patch("openhands_webhooks.app.db") as mock_db, \
         patch("openhands_webhooks.app.messaging") as mock_mq:
        mock_db.init_db = AsyncMock()
        mock_db.close_db = AsyncMock()
        mock_mq.connect = AsyncMock()
        mock_mq.disconnect = AsyncMock()
...
def test_gitlab_webhook_disabled(client):
    ...
    with patch("openhands_webhooks.handlers.gitlab.settings") as mock_settings:
...
def test_github_webhook_disabled(client):
    ...
    with patch("openhands_webhooks.handlers.github.settings") as mock_settings:
...
def test_jira_webhook_disabled(client):
    ...
    with patch("openhands_webhooks.handlers.jira.settings") as mock_settings:
...
def test_slack_webhook_disabled(client):
    ...
    with patch("openhands_webhooks.handlers.slack.settings") as mock_settings:
...
def test_link_conversation_missing_header(client):
    ...
    with patch("openhands_webhooks.app.require_relay_key", return_value=None):
        resp = client.post(
            "/conversations/link",
            json={
                "source": "gitlab",
