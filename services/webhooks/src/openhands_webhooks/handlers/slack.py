...
from openhands_common.db import PENDING_CONVERSATION_ID, is_pending

from ..config import require_relay_key, settings
from ..openhands_client import conversation_url, create_conversation, send_message
...
def _format_forwarded_message(
    comment_body: str, channel: str, has_mention: bool,
) -> str:
    if has_mention:
        preamble = (
...
def _response_instructions(channel: str) -> str:
    return (
        f"\n\n---\n"
...
async def _handle_mention(event: dict):
    ...
    if is_pending(existing):
        forward_msg = _format_forwarded_message(comment_text, channel, has_mention=True)
    ...
    reply_hint = _response_instructions(channel)
...
async def _handle_thread_message(event: dict):
    ...
    if is_pending(existing):
        forward_msg = _format_forwarded_message(comment_text, channel, has_mention=has_mention)
        await db.store_pending_message("slack", "thread", res_id, forward_msg)
        return
